...
export default function VouchComponent() {
  const { data: session, status } = useSession();
  const searchParams = useSearchParams();
  const userIdToVouch = searchParams.get("userId");
  const [isVouching, setIsVouching] = useState(false);

  // (Optional) Display a short user identifier
  ...
  const handleVouch = async () => {
    ...
    try {
      const response = await fetch("/api/user/vouch", {
        ...
        body: JSON.stringify({ userId: userIdToVouch }),
      });

      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "Failed to vouch for user.");

      toast.success("Vouch successful!", { description: data.message });
    } catch (error: any) {
      ...
      <Card className="w-full max-w-md text-center border-white/10 bg-white/[0.02]">
        ...
        <CardContent className="space-y-4">
          <p className="text-white/80">
            ...
            <span className="font-bold text-white">{userToVouchName}</span>?
          </p>
          <Button
            onClick={handleVouch}
          ...
          {status !== "authenticated" && (
            <p className="text-xs text-yellow-400">You must be signed in to vouch.</p>
          )}
        </CardContent>
      </Card>
    </div>
